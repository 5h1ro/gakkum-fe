...
import { useGetPascaPengawasanArsipQuery } from '../../../../api/pascaPengawasan.api';

function PascaPengawasanArsip() {
    const navigate = useNavigate();
    const [openFilter, setOpenFilter] = useState<boolean>(false)
    const filterExclude = ['aksi']
    const { data: arsip, isLoading: getting, isFetching } = useGetPascaPengawasanArsipQuery();
    ...
    const columns: MRT_ColumnDef<any>[] = [
        ...
        },
        {
            ...
            size: 50,
            Cell: ({ row }) => {
                return <IconButton onClick={() => navigate(`/pasca-pengawasan/arsip/detail/${row.original.id}`)} className="border-solid border-2 text-primary-600" aria-label="confirm">
                    <RiEyeLine />
                </IconButton>
            },
...
export default PascaPengawasanArsip;
