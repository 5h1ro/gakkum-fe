...
import { useGetDetailSumberDataQuery, useUpdateSumberDataMutation } from '../../../../api/setting.api';

interface updateSumberDataProps {
    showSnackBar: snackbarType
    setShowSnackbar: Dispatch<React.SetStateAction<snackbarType>>
}

export default function UpdateSumberData(props: updateSumberDataProps) {
    ...
    const { sumberDataID } = useParams();
    const [name, setName] = useState('')
    const [description, setDescription] = useState('')
    const [isActive, setIsActive] = useState<boolean>(true)

    const [updateSumberData, { isLoading }] = useUpdateSumberDataMutation();
    const handleSave = async () => {
        const formData = new FormData();
        formData.append('id', sumberDataID!);
        formData.append('name', name);
        formData.append('description', description);
        formData.append('is_active', `${isActive ? 1 : 0}`);
        formData.append('_method', 'PUT');
        try {
            await updateSumberData(formData).unwrap();
            setShowSnackbar({
                isOpen: true,
                message: 'Berhasil mengubah data',
                status: 'success'
            })
            navigate('/pengaturan/sumber-data')
        } catch (error: any) {
    ...
    const { data: dataDetailArtikel } = useGetDetailSumberDataQuery(sumberDataID!);
    useEffect(() => {
    ...
    return (
        <Layout>
            ...
            <Grid2 container alignContent={'center'}>
                <IconButton onClick={() => navigate('/pengaturan/sumber-data')}>
