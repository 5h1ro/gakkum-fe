...
import { useDeleteSumberDataMutation, useGetSumberDataQuery } from '../../../../api/setting.api';


interface sumberDataProps {
    showSnackBar: snackbarType
    setShowSnackbar: Dispatch<React.SetStateAction<snackbarType>>
}

function SumberData(props: sumberDataProps) {
    const [openDialog, setOpenDialog] = useState<boolean>(false);
    const { showSnackBar, setShowSnackbar } = props;
    const [sumberDataID, setSumberDataID] = useState('')
    const navigate = useNavigate();
    const [openFilter, setOpenFilter] = useState<boolean>(false)
    const [openFilterMobile, setOpenFilterMobile] = useState<boolean>(false)
    const filterExclude = ['report', 'aksi']
    const [deleteSumberData] = useDeleteSumberDataMutation();

    const { data: dataSumberData, isLoading: getting, isFetching } = useGetSumberDataQuery();
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                return <div className='flex gap-2'>
                    <IconButton className="border-solid border-2 text-primary-600" aria-label="confirm" onClick={() => navigate(`/pengaturan/sumber-data/detail/${row.original.id}`)}>
                        <RiEyeLine />
                    </IconButton>
                    <IconButton className="border-solid border-2 text-secondary-600" aria-label="confirm" onClick={() => navigate(`/pengaturan/sumber-data/update/${row.original.id}`)}>
                        <RiEditLine />
                    </IconButton>
                    <IconButton className="border-solid border-2 text-danger-600" aria-label="confirm" onClick={() => {
                        setSumberDataID(!dataSumberData ? '' : dataSumberData.data![row.index].id.toString())
                        setOpenDialog(true)
                    }}>
                        <RiDeleteBin7Line />
                    </IconButton>
        ...
        <Layout>
            ...
            <Grid2 container justifyContent={'space-between'}>
                ...
                <Grid2 container gap={2} className='w-full md:w-auto'>
                    <Button className="w-full mt-4 bg-primary-600 text-base-white rounded-lg py-4 px-6 hover:bg-primary-600 md:mt-0 md:w-auto gap-2" onClick={() => navigate('/pengaturan/sumber-data/create')}><RiAddLine /> Tambah</Button>
                </Grid2>
            </Grid2>
            ...
            <Grid2 container className="hidden md:block" marginTop={4}>
                <Box sx={{
                    ...
                    <Stack sx={{ margin: '24px' }}>
                        <Table openFilter={openFilter} setOpenFilter={setOpenFilter} columns={columns} data={dataSumberData?.data ?? []} state={{ isLoading: getting || isFetching }} filterExclude={filterExclude}></Table>
                    </Stack>
                </Box>
            </Grid2>
        </Layout>
