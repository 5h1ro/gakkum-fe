...
import { useGetDetailDokumenPerencanaanPengaturanQuery, useUpdateDokumenPerencanaanPengaturanMutation } from '../../../../api/setting.api';

interface updateDokumenPerencanaanProps {
    showSnackBar: snackbarType
    setShowSnackbar: Dispatch<React.SetStateAction<snackbarType>>
}

export default function UpdateDokumenPerencanaan(props: updateDokumenPerencanaanProps) {
    ...
    const { dokumenPerencanaanID } = useParams();
    const [name, setName] = useState('')
    const [description, setDescription] = useState('')
    const [isActive, setIsActive] = useState<boolean>(true)

    const [updateDokumenPerencanaan, { isLoading }] = useUpdateDokumenPerencanaanPengaturanMutation();
    const handleSave = async () => {
        const formData = new FormData();
        formData.append('id', dokumenPerencanaanID!);
        formData.append('name', name);
        formData.append('description', description);
        formData.append('is_active', `${isActive ? 1 : 0}`);
        formData.append('_method', 'PUT');
        try {
            await updateDokumenPerencanaan(formData).unwrap();
            setShowSnackbar({
                isOpen: true,
                message: 'Berhasil mengubah data',
                status: 'success'
            })
            navigate('/pengaturan/dokumen-perencanaan')
        } catch (error: any) {
    ...
    const { data: dataDokumenPerencanaan } = useGetDetailDokumenPerencanaanPengaturanQuery(dokumenPerencanaanID!);
    useEffect(() => {
        if (dataDokumenPerencanaan) {
            setName(dataDokumenPerencanaan.data?.name ?? '')
            setDescription(dataDokumenPerencanaan.data?.description ?? '')
            setIsActive(dataDokumenPerencanaan.data?.is_active ?? false)
        }
    }, [dataDokumenPerencanaan]);
    return (
        <Layout>
            <Snackbar
