...
import { useDeleteTahapanPerencanaanPengaturanMutation, useGetTahapanPerencanaanPengaturanQuery } from '../../../../api/setting.api';


interface tahapanPerencanaanProps {
    showSnackBar: snackbarType
    setShowSnackbar: Dispatch<React.SetStateAction<snackbarType>>
}

function TahapanPerencanaan(props: tahapanPerencanaanProps) {
    const [openDialog, setOpenDialog] = useState<boolean>(false);
    const { showSnackBar, setShowSnackbar } = props;
    const [tahapanPerencanaanID, setTahapanPerencanaanID] = useState('')
    const navigate = useNavigate();
    const [openFilter, setOpenFilter] = useState<boolean>(false)
    const [openFilterMobile, setOpenFilterMobile] = useState<boolean>(false)
    const filterExclude = ['report', 'aksi']
    const [deleteTahapanPerencanaan] = useDeleteTahapanPerencanaanPengaturanMutation();

    const { data: dataTahapanPerencanaan, isLoading: getting, isFetching } = useGetTahapanPerencanaanPengaturanQuery();
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                return <div className='flex gap-2'>
                    <IconButton className="border-solid border-2 text-primary-600" aria-label="confirm" onClick={() => navigate(`/pengaturan/tahapan-perencanaan/detail/${row.original.id}`)}>
                        <RiEyeLine />
                    </IconButton>
                    <IconButton className="border-solid border-2 text-secondary-600" aria-label="confirm" onClick={() => navigate(`/pengaturan/tahapan-perencanaan/update/${row.original.id}`)}>
                        <RiEditLine />
                    </IconButton>
                    <IconButton className="border-solid border-2 text-danger-600" aria-label="confirm" onClick={() => {
                        setTahapanPerencanaanID(!dataTahapanPerencanaan ? '' : dataTahapanPerencanaan.data![row.index].id.toString())
                        setOpenDialog(true)
                    }}>
                        <RiDeleteBin7Line />
                    </IconButton>
                ...
                <DialogContent>
                    ...
                    <Grid2 container className='flex gap-2 mt-6 justify-center pb-8 md:pb-16'>
                        <Button className='bg-danger-500 text-base-white hover:bg-danger-500 hover:text-base-white py-4 px-6 rounded-xl' onClick={() => {
                            try {
                                deleteTahapanPerencanaan(tahapanPerencanaanID)
                                setOpenDialog(false)
                                setShowSnackbar({
                                    isOpen: true,
            ...
            <Grid2 container justifyContent={'space-between'}>
                <Grid2 container alignContent={'center'}>
                    <Typography className="text-3xl font-semibold text-base-dark mt-[-2px] md:mt-0 md:text-4xl">
                        Manajemen Tahapan Perencanaan
                    </Typography>
                </Grid2>
                <Grid2 container gap={2} className='w-full md:w-auto'>
                    <Button className="w-full mt-4 bg-primary-600 text-base-white rounded-lg py-4 px-6 hover:bg-primary-600 md:mt-0 md:w-auto gap-2" onClick={() => navigate('/pengaturan/tahapan-perencanaan/create')}><RiAddLine /> Tambah</Button>
                </Grid2>
            </Grid2>
            ...
            <Grid2 container className="hidden md:block" marginTop={4}>
                <Box sx={{
                    ...
                    <Stack sx={{ margin: '24px' }}>
                        <Table openFilter={openFilter} setOpenFilter={setOpenFilter} columns={columns} data={dataTahapanPerencanaan?.data ?? []} state={{ isLoading: getting || isFetching }} filterExclude={filterExclude}></Table>
                    </Stack>
                </Box>
            </Grid2>
        </Layout>
...
export default TahapanPerencanaan;
