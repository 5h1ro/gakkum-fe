...
import { useGetDetailTahapanPerencanaanPengaturanQuery, useUpdateTahapanPerencanaanPengaturanMutation } from '../../../../api/setting.api';

interface updateTahapanPerencanaanProps {
    showSnackBar: snackbarType
    setShowSnackbar: Dispatch<React.SetStateAction<snackbarType>>
}

export default function UpdateTahapanPerencanaan(props: updateTahapanPerencanaanProps) {
    ...
    const { tahapanPerencanaanID } = useParams();
    const [name, setName] = useState('')
    const [description, setDescription] = useState('')
    const [isActive, setIsActive] = useState<boolean>(true)

    const [updateTahapanPerencanaan, { isLoading }] = useUpdateTahapanPerencanaanPengaturanMutation();
    const handleSave = async () => {
        const formData = new FormData();
        formData.append('id', tahapanPerencanaanID!);
        formData.append('name', name);
        formData.append('description', description);
        formData.append('is_active', `${isActive ? 1 : 0}`);
        formData.append('_method', 'PUT');
        try {
            await updateTahapanPerencanaan(formData).unwrap();
            setShowSnackbar({
                isOpen: true,
    ...
    const { data: dataTahapanPerencanaan } = useGetDetailTahapanPerencanaanPengaturanQuery(tahapanPerencanaanID!);
    useEffect(() => {
        if (dataTahapanPerencanaan) {
            setName(dataTahapanPerencanaan.data?.name ?? '')
            setDescription(dataTahapanPerencanaan.data?.description ?? '')
            setIsActive(dataTahapanPerencanaan.data?.is_active ?? false)
        }
    }, [dataTahapanPerencanaan]);
    return (
        <Layout>
            <Snackbar
