...
import { useDeleteEmployeePengaturanMutation, useGetEmployeePengaturanQuery } from '../../../../api/setting.api';
...
function Employee(props: employeeProps) {
    ...
    const [deleteEmployee] = useDeleteEmployeePengaturanMutation();

    const { data: dataEmployee, isLoading: getting, isFetching } = useGetEmployeePengaturanQuery();
    const columns: MRT_ColumnDef<any>[] = [
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                return <Typography>{row.original.position}</Typography>
            },
            filterFn: 'equals',
            filterVariant: 'select',
            muiFilterTextFieldProps: {
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                return <Typography>{row.original.dob}</Typography>
            },
            filterFn: 'equals',
            filterVariant: 'select',
            muiFilterTextFieldProps: {
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                return <Typography>{row.original.pob}</Typography>
            },
            filterFn: 'equals',
            filterVariant: 'select',
            muiFilterTextFieldProps: {
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                let status_data
                switch (row.original.user.is_active) {
                    case 1:
                        status_data = {
                            status: 'Active',
                            style: {
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                return <div className='flex gap-2'>
                    <IconButton className="border-solid border-2 text-primary-600" aria-label="confirm" onClick={() => navigate(`/pengaturan/user/detail/${row.original.id}`)}>
                        <RiEyeLine />
                    </IconButton>
                    <IconButton className="border-solid border-2 text-secondary-600" aria-label="confirm" onClick={() => navigate(`/pengaturan/user/update/${row.original.id}`)}>
                        <RiEditLine />
                    </IconButton>
                    <IconButton className="border-solid border-2 text-danger-600" aria-label="confirm" onClick={() => {
                        setEmployeeID(!dataEmployee ? '' : dataEmployee.data![row.index].id.toString())
                        setOpenDialog(true)
                    }}>
                        <RiDeleteBin7Line />
                    </IconButton>
        ...
        <Layout>
            ...
            <Grid2 container justifyContent={'space-between'}>
                ...
                <Grid2 container gap={2} className='w-full md:w-auto'>
                    <Button className="w-full mt-4 bg-primary-600 text-base-white rounded-lg py-4 px-6 hover:bg-primary-600 md:mt-0 md:w-auto gap-2" onClick={() => navigate('/pengaturan/user/create')}><RiAddLine /> Tambah</Button>
                </Grid2>
            </Grid2>
            ...
            <Grid2 container className="hidden md:block" marginTop={4}>
                <Box sx={{
                    ...
                    <Stack sx={{ margin: '24px' }}>
                        <Table openFilter={openFilter} setOpenFilter={setOpenFilter} columns={columns} data={dataEmployee?.data ?? []} state={{ isLoading: getting || isFetching }} filterExclude={filterExclude}></Table>
                    </Stack>
                </Box>
            </Grid2>
        </Layout>
