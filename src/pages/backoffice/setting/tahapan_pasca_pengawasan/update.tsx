...
import { useGetDetailTahapanPascaPengawasanPengaturanQuery, useUpdateTahapanPascaPengawasanPengaturanMutation } from '../../../../api/setting.api';

interface updateTahapanPascaPengawasanProps {
    showSnackBar: snackbarType
    setShowSnackbar: Dispatch<React.SetStateAction<snackbarType>>
}

export default function UpdateTahapanPascaPengawasan(props: updateTahapanPascaPengawasanProps) {
    ...
    const { tahapanPascaPengawasanID } = useParams();
    const [name, setName] = useState('')
    const [description, setDescription] = useState('')
    const [isActive, setIsActive] = useState<boolean>(true)

    const [updateTahapanPascaPengawasan, { isLoading }] = useUpdateTahapanPascaPengawasanPengaturanMutation();
    const handleSave = async () => {
        const formData = new FormData();
        formData.append('id', tahapanPascaPengawasanID!);
        formData.append('name', name);
        formData.append('description', description);
        formData.append('is_active', `${isActive ? 1 : 0}`);
        formData.append('_method', 'PUT');
        try {
            await updateTahapanPascaPengawasan(formData).unwrap();
            setShowSnackbar({
                isOpen: true,
                message: 'Berhasil mengubah data',
                status: 'success'
            })
            navigate('/pengaturan/tahapan-pasca-pengawasan')
        } catch (error: any) {
    ...
    const { data: dataTahapanPascaPengawasan } = useGetDetailTahapanPascaPengawasanPengaturanQuery(tahapanPascaPengawasanID!);
    useEffect(() => {
        if (dataTahapanPascaPengawasan) {
            setName(dataTahapanPascaPengawasan.data?.name ?? '')
            setDescription(dataTahapanPascaPengawasan.data?.description ?? '')
            setIsActive(dataTahapanPascaPengawasan.data?.is_active ?? false)
        }
    }, [dataTahapanPascaPengawasan]);
    return (
        <Layout>
            <Snackbar
