...
import { useGetDetailStatusDataQuery, useUpdateStatusDataMutation } from '../../../../api/setting.api';

interface updateStatusDataProps {
    showSnackBar: snackbarType
    setShowSnackbar: Dispatch<React.SetStateAction<snackbarType>>
}

export default function UpdateStatusData(props: updateStatusDataProps) {
    ...
    const { statusDataID } = useParams();
    const [name, setName] = useState('')
    const [description, setDescription] = useState('')
    const [isActive, setIsActive] = useState<boolean>(true)

    const [updateStatusData, { isLoading }] = useUpdateStatusDataMutation();
    const handleSave = async () => {
        const formData = new FormData();
        formData.append('id', statusDataID!);
        formData.append('name', name);
        formData.append('description', description);
        formData.append('is_active', `${isActive ? 1 : 0}`);
        formData.append('_method', 'PUT');
        try {
            await updateStatusData(formData).unwrap();
            setShowSnackbar({
                isOpen: true,
                message: 'Berhasil mengubah data',
                status: 'success'
            })
            navigate('/pengaturan/status-data')
        } catch (error: any) {
    ...
    const { data: dataDetailArtikel } = useGetDetailStatusDataQuery(statusDataID!);
    useEffect(() => {
        if (dataDetailArtikel) {
            setName(dataDetailArtikel.data?.name ?? '')
            setDescription(dataDetailArtikel.data?.description ?? '')
            setIsActive(dataDetailArtikel.data?.is_active ?? false)
        }
    }, [dataDetailArtikel]);
    return (
        <Layout>
            ...
            <Grid2 container alignContent={'center'}>
                <IconButton onClick={() => navigate('/pengaturan/status-data')}>
