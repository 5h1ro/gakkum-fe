...
import { useGetDetailTahapanPengawasanPengaturanQuery, useUpdateTahapanPengawasanPengaturanMutation } from '../../../../api/setting.api';

interface updateTahapanPengawasanProps {
    showSnackBar: snackbarType
    setShowSnackbar: Dispatch<React.SetStateAction<snackbarType>>
}

export default function UpdateTahapanPengawasan(props: updateTahapanPengawasanProps) {
    ...
    const { tahapanPengawasanID } = useParams();
    const [name, setName] = useState('')
    const [description, setDescription] = useState('')
    const [isActive, setIsActive] = useState<boolean>(true)

    const [updateTahapanPengawasan, { isLoading }] = useUpdateTahapanPengawasanPengaturanMutation();
    const handleSave = async () => {
        const formData = new FormData();
        formData.append('id', tahapanPengawasanID!);
        formData.append('name', name);
        formData.append('description', description);
        formData.append('is_active', `${isActive ? 1 : 0}`);
        formData.append('_method', 'PUT');
        try {
            await updateTahapanPengawasan(formData).unwrap();
            setShowSnackbar({
                isOpen: true,
                message: 'Berhasil mengubah data',
                status: 'success'
            })
            navigate('/pengaturan/tahapan-pengawasan')
        } catch (error: any) {
    ...
    const { data: dataTahapanPengawasan } = useGetDetailTahapanPengawasanPengaturanQuery(tahapanPengawasanID!);
    useEffect(() => {
        if (dataTahapanPengawasan) {
            setName(dataTahapanPengawasan.data?.name ?? '')
            setDescription(dataTahapanPengawasan.data?.description ?? '')
            setIsActive(dataTahapanPengawasan.data?.is_active ?? false)
        }
    }, [dataTahapanPengawasan]);
    return (
        <Layout>
            <Snackbar
