...
import { useGetDetailDokumenPascaPengawasanPengaturanQuery, useUpdateDokumenPascaPengawasanPengaturanMutation } from '../../../../api/setting.api';

interface updateDokumenPascaPengawasanProps {
    showSnackBar: snackbarType
    setShowSnackbar: Dispatch<React.SetStateAction<snackbarType>>
}

export default function UpdateDokumenPascaPengawasan(props: updateDokumenPascaPengawasanProps) {
    ...
    const [updateDokumenPascaPengawasan, { isLoading }] = useUpdateDokumenPascaPengawasanPengaturanMutation();
    const handleSave = async () => {
        const formData = new FormData();
        formData.append('id', dokumenPascaPengawasanID!);
        formData.append('name', name);
        formData.append('description', description);
        formData.append('is_active', `${isActive ? 1 : 0}`);
        formData.append('_method', 'PUT');
        try {
            await updateDokumenPascaPengawasan(formData).unwrap();
            setShowSnackbar({
                isOpen: true,
                message: 'Berhasil mengubah data',
                status: 'success'
            })
            navigate('/pengaturan/dokumen-pasca-pengawasan')
        } catch (error: any) {
        ...
        <Layout>
            ...
            <Grid2 container alignContent={'center'}>
                ...
                <Typography className="w-10/12 text-4xl font-semibold text-base-dark pt-1">
                    Update Dokumen Pasca Pengawasan
                </Typography>
            </Grid2>
