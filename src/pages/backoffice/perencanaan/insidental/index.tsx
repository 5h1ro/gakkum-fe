...
import { useGetPerencanaanInsidentalQuery } from '../../../../api/perencanaan.api';

function PerencanaanInsidental() {
    const navigate = useNavigate();
    const [openFilter, setOpenFilter] = useState<boolean>(false)
    const filterExclude = ['aksi']
    const { data: insidental, isLoading: getting, isFetching } = useGetPerencanaanInsidentalQuery();
            ...
            <Grid2 container className="hidden md:block" marginTop={4}>
                <Box sx={{
                    ...
                    <Stack sx={{ margin: '24px' }}>
                        <Table openFilter={openFilter} setOpenFilter={setOpenFilter} columns={columns} data={insidental?.data ?? []} filterExclude={filterExclude}></Table>
                    </Stack>
                </Box>
            </Grid2>
        </Layout>
...
export default PerencanaanInsidental;
