...
import { useGetRegulerQuery } from '../../../../api/register.api';

function RegistrasiReguler() {
    const navigate = useNavigate();
    const [openFilter, setOpenFilter] = useState<boolean>(false)
    const filterExclude = ['aksi']
    const { data: reguler, isLoading: getting, isFetching } = useGetRegulerQuery();
            ...
            <Grid2 container className="hidden md:block" marginTop={4}>
                <Box sx={{
                    ...
                    <Stack sx={{ margin: '24px' }}>
                        <Table openFilter={openFilter} setOpenFilter={setOpenFilter} columns={columns} data={reguler?.data ?? []} state={{ isLoading: getting || isFetching }} filterExclude={filterExclude}></Table>
                    </Stack>
                </Box>
            </Grid2>
        </Layout>
...
export default RegistrasiReguler;
