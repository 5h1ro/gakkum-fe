...
import { useGetPengawasanArsipQuery } from '../../../../api/pengawasan.api';

function PengawasanArsip() {
    const navigate = useNavigate();
    const [openFilter, setOpenFilter] = useState<boolean>(false)
    const filterExclude = ['aksi']
    const { data: arsip, isLoading: getting, isFetching } = useGetPengawasanArsipQuery();
    ...
    const columns: MRT_ColumnDef<any>[] = [
        ...
        },
        {
            ...
            size: 50,
            Cell: ({ row }) => {
                return <IconButton onClick={() => navigate(`/pengawasan/arsip/detail/${row.original.id}`)} className="border-solid border-2 text-primary-600" aria-label="confirm">
                    <RiEyeLine />
                </IconButton>
            },
